//// ============================================================
////  LocationMonitorService.kt — FIXED VERSION
////
////  POLYLINE FIXES:
////    1. BULK_CAPTURE_MS: 1s → 10s (drastically reduces noise points)
////    2. Accuracy filter: skip if GPS accuracy > 50m
////    3. Minimum distance filter: skip if moved < 10m from last point
////    4. Offline file persistence: buffer saved to disk on kill/destroy
////    5. Offline buffer loaded on startup and included in next API post
////
...
class LocationMonitorService : Service() {
    ...
    private fun checkLocationAndPermission() {
        ...
        if (cachedEndTime.isNotEmpty()) {
            try {
                // ✅ FIX: Overtime employees bhi shift end pe clockout honge —
                // overtime flag sirf clock-in/out blocking ke liye tha, auto clockout ke liye nahi.
                val parsed = parseTimeTo24h(cachedEndTime)
                if (parsed != null) {
                    val endTotalMin = parsed.first * 60 + parsed.second
                    val nowTotalMin = hour * 60 + minute
    ...
    override fun onTaskRemoved(rootIntent: Intent?) {
        ...
        if (clocked && !frozen) {
            ...
            // ✅ FIX: Overtime check hata diya — sab employees ka shift end pe clockout hoga
    ...
    private fun scheduleShiftEndAlarm() {
        try {
            ...
            // ✅ FIX: Overtime check hata diya — sab employees ka shift end pe alarm fire hoga
