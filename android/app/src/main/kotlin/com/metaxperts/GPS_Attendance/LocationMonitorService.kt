...
//    private val FAKE_GPS_COOLDOWN_MS = 30_000L // 30 seconds
//
//    companion object {
//        fun start(context: Context) {
//            try {
...
//        if (now - lastFakeGpsReportTime < FAKE_GPS_COOLDOWN_MS) return // cooldown
//        lastFakeGpsReportTime = now
//
//        debugPrint("🚨 [FakeGPS] Mock location detected! lat=${loc.latitude} lon=${loc.longitude}")
//
//        val prefs       = getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
//        val empId       = prefs.getString("flutter.emp_id", "")       ?: ""
//        val empName     = prefs.getString("flutter.emp_name", "")     ?: ""
//        val companyCode = prefs.getString("flutter.company_code", "") ?: ""
...
//        }.toString()
//
//        // POST on a background thread — never block the location callback
//        Thread {
//            try {
...
//                debugPrint("✅ [FakeGPS] POST → $code")
//                conn.disconnect()
//            } catch (e: Exception) {
//                debugPrint("❌ [FakeGPS] POST failed: ${e.message}")
//                // Flutter side will retry via syncPending() on next app open
//            }
//        }.start()
//    }
//
...
//    1. Identity (deviceId, companyCode, empName) read from Intent extras
//       first, then FlutterSharedPreferences as fallback (service restart).
//    2. MQTT topic is now dynamic: gps/$companyCode/$deviceId
//       (was hardcoded "gps/location")
//    3. buildPayload() now includes company_code + emp_name fields.
//    4. isCleanSession = false  (session persists across reconnects)
//    5. keepAliveInterval = 60  (was 30)
//    6. start() companion overload accepts optional identity extras.
// ============================================================
