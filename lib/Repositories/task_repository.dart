...
class TaskRepository {
  ...
  // ✅ NEW: Create Task ke liye static URL — Firebase Remote Config se NAHI aata,
  // hardcoded rakha gaya hai jaisa request kiya gaya tha.
  static const String _createTaskStaticUrl =
      'http://oracle.metaxperts.net/ords/gps_workforce/taskmanager/post/';

  // ✅ NEW: GET Assigned Tasks (TaskManager Detail) ke liye static URL — Firebase
  // Remote Config se NAHI aata, hardcoded rakha gaya hai jaisa request kiya gaya tha.
  static const String _getTasksStaticUrl =
      'http://oracle.metaxperts.net/ords/gps_workforce/taskmanagerdetail/get/';

  // ✅ NEW: PUT Update Task ke liye static URL
  static const String _updateTaskStaticUrl =
      'http://oracle.metaxperts.net/ords/gps_workforce/taskmanagerupdate/put/';
  ...
  // ── NEW: Mapper for the TaskManagerDetail (TM_TASKS) response shape ──────
  // Column names here differ from the legacy /task/get response
  // (TASK_ID, TASK_NAME, DESCRIPTION, ... instead of id, task_title, ...),
  // so it gets its own mapper instead of reusing _taskFromMap.
  TaskModel _taskFromTaskManagerMap(Map<String, dynamic> m) {
    return TaskModel(
      id: (_val(m, 'task_id') as num?)?.toInt() ?? 0,
      empId: (_val(m, 'emp_id') as num?)?.toInt() ?? 0,
      empName: _val(m, 'emp_name')?.toString() ?? '',
      taskTitle: _val(m, 'task_name')?.toString() ?? '',
      taskDescription: _val(m, 'description')?.toString() ?? '',
      status: _val(m, 'status')?.toString() ?? 'Pending',
      priority: _val(m, 'priority')?.toString() ?? 'Medium',
      dueDate: _val(m, 'due_date')?.toString() ?? '',
      comments: '',     // ← TaskManagerDetail query doesn't return this field
      assignedBy: '',    // ← TaskManagerDetail query doesn't return this field
      createdAt: _val(m, 'created_at')?.toString() ?? '',
      taskType: _val(m, 'task_type')?.toString() ?? 'SELF',
      category: '',      // ← TaskManagerDetail query doesn't return this field
      company_code: _val(m, 'company_code')?.toString(),
    );
  }

  // ── NEW: List parser for the TaskManagerDetail response ──────────────────
  List<TaskModel> _parseTaskManagerList(String body) {
    debugPrint('═══════ RAW ORACLE RESPONSE (TaskManagerDetail) ═══════');
    debugPrint(body);
    debugPrint('═══════════════════════════════════');
    ...
    for (int i = 0; i < rawList.length; i++) {
      final item = rawList[i] as Map<String, dynamic>;
      final task = _taskFromTaskManagerMap(item);
      debugPrint('✅ Parsed ID=${task.id}  title="${task.taskTitle}"');
      tasks.add(task);
    }
    return tasks;
  ...
  Future<RepositoryResult<List<TaskModel>>> getAssignedTasks() async {
    try {
      ...
      final uri = Uri.parse(_getTasksStaticUrl)
      ...
      if (response.statusCode == 200) {
        return RepositoryResult.success(_parseTaskManagerList(response.body));
      }
  ...
  Future<RepositoryResult<TaskModel>> createTask(CreateTaskRequest request) async {
    try {
      ...
      debugPrint('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      debugPrint('📡 [POST] URL  : $_createTaskStaticUrl');
      debugPrint('📡 [POST] Body : ${jsonEncode(body)}');
      debugPrint('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');

      final response = await http.post(
        Uri.parse(_createTaskStaticUrl),
        headers: headers,
  ...
  // ══════════════════════════════════════════════════════════════════════════
  //  PUT update task
  //  ✅ NOW using static URL + sends task_id in body
  //  ✅ UPDATED: Using Remote Config for timeout
  // ══════════════════════════════════════════════════════════════════════════
