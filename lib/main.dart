...
import 'Screens/sync_status_card_screen.dart';
...
// ── ✅ SYNC CONTROLLER ──────────────────────────────────────────────────────
// Placeholder — you'll replace with actual SyncController import
...
void main() {
  runZonedGuarded(() async {
    ...
    // ✅ FRESH INSTALL DETECTION (single, clean — BEFORE PowerOffService)
    final isAppInstalled = prefs.getBool('app_installed_flag') ?? false;
    if (!isAppInstalled) {
      // PowerOff events are already captured above, so safe to clear
      await prefs.clear();
    ...
    // ── ✅ SYNC CONTROLLER ── OFFLINE DATA SYNC ──────────────────────────
    // This will auto-sync visits, orders, battery, fakegps, poweroff
    // Get.put(SyncController(), permanent: true);
    // debugPrint('🔄 [SyncController] Registered in main');
    ...
    if (isAuthenticated) {
      ...
      // ✅ Sync device storage info (7-day interval, online only)
      unawaited(DeviceInfoService.syncDeviceInfoIfNeeded());
    }
    ...
    // ✅ Unconditional cold-start drain of the bulk-location SQLite queue.
    // Level-triggered replacement for the old edge-triggered path
    // (which only fired on an offline→online transition — the reason a reboot
    // "fixed" it). Runs regardless of clock-in; no-ops if offline or queue empty.
    unawaited(LocationBulkTracker.instance.syncPendingRecords());

    debugPrint("Running the app...");
    runApp(const MyApp());

  }, (error, stack) {
    // ✅ Zone-level error catcher — Firebase + Oracle both
    FirebaseCrashlytics.instance.recordError(error, stack, fatal: true);
    CrashLogService.postCrashToServer(
      error: error.toString(),
      stack: stack.toString(),
...
class AppLifecycleObserver extends NavigatorObserver {
  ...
  void _checkAndShowDialog(Route route) async {
    if (route.settings.name == routeHome) {
      ...
      final alreadyShownOnce =
          prefs.getBool('setup_dialog_shown_once') ?? false;

      // Skip if already acknowledged OR shown once (prevents re-show on back navigation)
      if (alreadyAcknowledged || alreadyShownOnce) {
        debugPrint('✅ [Observer] Setup dialog already shown/acknowledged, skipping...');
        return;
      }
      ...
      Future.delayed(const Duration(milliseconds: 500), () {
        ...
        if (context != null && context.mounted) {
          ...
          // Mark as shown IMMEDIATELY — before user even responds
          prefs.setBool('setup_dialog_shown_once', true);
...
// ── ✅ AppSetupDialog (Placeholder) ──────────────────────────────────────
// You'll need to define this class or import it from its actual file
class AppSetupDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      content: const Text('Please complete the setup to continue.'),
      actions: [
        TextButton(
          onPressed: () async {
            final prefs = await SharedPreferences.getInstance();
