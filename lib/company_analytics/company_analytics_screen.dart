...
import 'attendance_analytics_service_screen.dart';
import 'daily_attendance_report_service.dart';
import 'task_stats_service.dart';
...
class _CompanyAnalyticsTabState extends State<CompanyAnalyticsTab> {
  ...
  // ── Task Stats (real data, shown in the Tasks sub-tab) ──────────────────
  late TaskStatsService _taskStatsService;
  TaskStatsData? _taskStatsData;
  bool _isTaskStatsLoading = false;
  ...
  // ── Fetch task stats from API (no month param — see task_stats_service) ─
  // Future<void> _fetchTaskStatsData() async {
  //   setState(() => _isTaskStatsLoading = true);
  //
  //   final data = await _taskStatsService.fetchTaskStats();
  //
  //   setState(() {
  //     _taskStatsData = data;
  //     _isTaskStatsLoading = false;
  //   });
  // }

  // ── Fetch task stats from API (now with year & month) ─────────────────
  Future<void> _fetchTaskStatsData() async {
    setState(() => _isTaskStatsLoading = true);

    // Format as "YYYY-MM" instead of separate year and month
    final month = DateFormat('yyyy-MM').format(_analyticsMonth);  // "2026-06"

    final data = await _taskStatsService.fetchTaskStats(
      month: month,  // Just one parameter now
    );
  ...
  // ── Simple View / Full Detail View toggle ───────────────────────────────
  Widget _buildViewToggle() {
    // Compliance and Tasks have no separate "full detail" breakdown yet —
    // only show the Simple View option for those sub-tabs.
    final showFullDetailOption = _analyticsSubTab != _AnalyticsSubTab.compliance &&
        _analyticsSubTab != _AnalyticsSubTab.tasks;
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16),
      child: Container(
        padding: const EdgeInsets.all(4),
  ...
  Widget _buildAnalyticsContent() {
    if (_analyticsSubTab == _AnalyticsSubTab.tasks) {
      ...
      return _buildTaskStatsView(_taskStatsData!);
    }
  ...
  // ── Tasks view: Total / Completed / In Progress / Pending / Overdue
  // Sourced from the taskstats API. Same stat cards for both Simple View
  // and Full Detail View for now (no separate per-task breakdown endpoint
  // exists yet) — only the trailing list differs.
  // Widget _buildTaskStatsView(TaskStatsData data) {
  //   final taskStats = [
  //     _AttendanceStat(
  ...
  //         ),
  //         itemBuilder: (context, index) =>
  //             _buildAttendanceStatCard(taskStats[index]),
  //       ),
  //       const SizedBox(height: 16),
  //       _buildTaskCompletionCard(data),
  //     ],
  //   );
  // }


  // ── Tasks view: Total / Completed / In Progress / Pending / Overdue / Avg Completion
  Widget _buildTaskStatsView(TaskStatsData data) {
    final taskStats = [
      _AttendanceStat(
        icon: Icons.checklist_rounded,
  ...
  // ── Task Completion card (mirrors the Attendance Score card) ───────────
  // ── Task Completion card (mirrors the Attendance Score card) ───────────
  Widget _buildTaskCompletionCard(TaskStatsData data) {
    final progress = data.completionScore / 100;
    return ClipRRect(
      borderRadius: BorderRadius.circular(14),
      child: Container(
        padding: const EdgeInsets.all(14),
        decoration: BoxDecoration(
          ...
          // borderRadius: BorderRadius.circular(14), // REMOVE THIS LINE
        ),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          mainAxisSize: MainAxisSize.min,
