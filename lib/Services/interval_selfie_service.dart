...
class IntervalSelfieService extends GetxController with WidgetsBindingObserver {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // API FETCH  — uses RemoteConfigService for URL
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  Future<void> _fetchPolicyFromApi() async {
    ...
    try {
      final url = RemoteConfigService.getPresenceCheckGetUrl(_empId, _companyCode);
      ...
      if (response.statusCode == 200) {
        ...
        if (item != null) {
          ...
          _notifCount       = newCount;
          _notifTimeMinutes = newTimeMin;
          if (empName.toString().isNotEmpty) _empName = empName.toString();
  ...
  // SELFIE UPLOAD — API POST + OFFLINE QUEUE
  //
  // Endpoint (via RemoteConfigService.getPresenceCheckPostSelfieUrl):
  //   POST <api_base_url>/presencecheckpost/selfie/
  //   Content-Type: image/jpeg
  //   Body: raw image bytes  (ORDS :body → SELFIE_IMAGE BLOB)
  //   Query: emp_id, emp_name, company_code, image_mime_type,
  //          latitude, longitude, captured_at
  //
  // Offline behaviour:
  //   • If POST fails → row saved to selfie_log (posted=0).
  ...
  /// HTTP POST via RemoteConfigService URL (no hardcoded base URL).
  Future<bool> _postSelfieToApi({
    required String    empId,
    required String    empName,
    required String    companyCode,
    ...
    // Build URL via RemoteConfigService
    final uri = Uri.parse(RemoteConfigService.getPresenceCheckPostSelfieUrl(
      empId:        empId,
      empName:      empName,
    ...
    try {
      ...
      final response = await http
        ...
        onTimeout: () {
          ...
          debugPrint('══════════════════════════════════════════════════════');
          debugPrint('⏱️ [SELFIE POST] TIMEOUT — no response after 20s');
          debugPrint('⏱️ [SELFIE POST]   Server: ${RemoteConfigService.getApiBaseUrl()}');
          debugPrint('⏱️ [SELFIE POST]   Selfie will be saved offline and retried');
          debugPrint('══════════════════════════════════════════════════════');
          debugPrint('');
          throw TimeoutException('Selfie POST timed out after 20s', const Duration(seconds: 20));
    ...
    try {
      ...
      for (final row in unposted) {
        ...
        final success = await _postSelfieToApi(
          ...
          empName:     row['emp_name']?.toString()     ?? '',
          companyCode: row['company_code']?.toString() ?? '',
          imageBytes:  base64Decode(row['selfie_image']?.toString() ?? ''),
          lat:         (row['latitude']  as num?)?.toDouble() ?? 0.0,
          lng:         (row['longitude'] as num?)?.toDouble() ?? 0.0,
          capturedAt:  row['captured_at']?.toString()  ?? '',
        );
