...
class SalarySlipViewModel extends GetxController {
  ...
  // ✅ UPDATED: Using Remote Config for salary slip URL
  Future<void> fetchSalarySlips() async {
    if (empId.value.isEmpty) {
      errorMessage.value = 'Employee ID is empty. Please log in again.';
      return;
    ...
    try {
      ...
      // ✅ FIX: Handle nullable return from DBHelper.getCompanyCode()
      final String? companyCodeNullable = DBHelper.getCompanyCode();
      final String companyCode = companyCodeNullable ?? '';

      final uri = Uri.parse(RemoteConfigService.getSalarySlipUrl(empId.value, companyCode));
      ...
      if (response.statusCode == 200) {
        ...
        if (items.isEmpty) {
          errorMessage.value = 'API responded OK but returned 0 records.\n\n'
              'Full response:\n${response.body}';
        } else {
          slips.value = items
              .map((e) => SalarySlip.fromJson(e as Map<String, dynamic>))
