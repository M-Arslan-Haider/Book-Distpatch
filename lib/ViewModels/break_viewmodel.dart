...
class BreakViewModel extends GetxController {
  ...
  // ── Day name helper ───────────────────────────────────────────────────────
  // DateTime.weekday: 1=Monday ... 7=Sunday — same order as cached_shift_schedule
  ...
  // ── Per-day break time from cached shift schedule ─────────────────────────
  // cached_shift_schedule (set by login_repository) mein aaj ke din ka
  // break_start aur break_end dhundo.
  ...
  // Already-logged-in users ka old cache (bina break fields ke) detect hone
  // par silently refresh karta hai. Next poll pe automatically kaam karega.
  void _triggerScheduleRefresh() {
    // _userId already loaded in _loadUserId() — prefs se dobara read karne ki zaroorat nahi
    if (_userId.isEmpty) {
      debugPrint('[BreakSchedule] ⚠️ Refresh skipped — _userId is empty');
      return;
    }
    SharedPreferences.getInstance().then((prefs) {
      // Company code: prefCompanyCode key (same as login_view_model uses)
      final company = prefs.getString(prefCompanyCode)
          ?? prefs.getString('cached_employees_company')
          ?? '';
      if (company.isEmpty) {
        debugPrint('[BreakSchedule] ⚠️ Refresh skipped — company empty');
        return;
      }
      debugPrint('[BreakSchedule] 🔄 Triggering background schedule refresh for emp=$_userId company=$company');
      Get.find<LoginRepository>()
          .refreshShiftSchedule(_userId, company)
          .then((_) async {
        debugPrint('[BreakSchedule] ✅ Background schedule refresh complete — applying fresh break time');
        await _applyTodayBreakFromSchedule();
      })
          .catchError((e) {
        debugPrint('[BreakSchedule] ⚠️ Background refresh failed: $e');
        return null;
      });
    });
  }

  Future<bool> _applyTodayBreakFromSchedule() async {
    try {
      debugPrint('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      debugPrint('[BreakSchedule] 🔍 _applyTodayBreakFromSchedule() called');

      final prefs = await SharedPreferences.getInstance();
      final raw   = prefs.getString('cached_shift_schedule') ?? '';
      ...
      debugPrint('[BreakSchedule] 📦 cached_shift_schedule found (${raw.length} chars)');

      final Map<String, dynamic> schedule = jsonDecode(raw);
      final String todayName = _todayDayName();

      debugPrint('[BreakSchedule] 📅 Today = "$todayName"');
      debugPrint('[BreakSchedule] 📅 Available days in cache: ${schedule.keys.toList()}');

      final dynamic todayRaw = schedule[todayName];

      if (todayRaw == null || todayRaw is! Map) {
        debugPrint('[BreakSchedule] ❌ "$todayName" NOT found in 7-day schedule — falling back to attendancedata API');
        debugPrint('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        return false;
      }

      final Map<String, dynamic> todayData = Map<String, dynamic>.from(todayRaw);
      debugPrint('[BreakSchedule] ✅ "$todayName" found in schedule: $todayData');

      final String bStart = (todayData['break_start'] ?? '').toString().trim();
      final String bEnd   = (todayData['break_end']   ?? '').toString().trim();

      debugPrint('[BreakSchedule] 🕐 Raw break_start="$bStart"  break_end="$bEnd"');
      ...
      final String startTime = _normalizeTime(bStart);
      final String endTime   = _normalizeTime(bEnd);

      debugPrint('[BreakSchedule] 🕐 Normalized → start="$startTime"  end="$endTime"');

      if (startTime.isEmpty || endTime.isEmpty) {
        debugPrint('[BreakSchedule] ❌ Normalization failed for "$todayName" ($bStart–$bEnd) — falling back to attendancedata API');
        debugPrint('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        return false;
      }

      scheduledBreak.value = ScheduledBreak(breakStart: startTime, breakEnd: endTime);
      ...
      debugPrint('[BreakSchedule] ✅ Day-wise break applied for "$todayName": $startFmt – $endFmt');
      debugPrint('[BreakSchedule] 💾 Saved to prefs → start=$startTime  end=$endTime');
      debugPrint('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      return true;
    } catch (e) {
      debugPrint('[BreakSchedule] ⚠️ _applyTodayBreakFromSchedule error: $e');
      debugPrint('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      return false;
    }
  }
  ...
  Future<void> fetchScheduledBreakTime() async {
    try {
      // ── Periodic background refresh of shift schedule ─────────────────────
      // Har 30 min mein silently shiftdetails API se latest break times fetch
      // karo — admin ka koi bhi change next cycle mein reflect hoga.
      _maybeRefreshScheduleInBackground();
      // ─────────────────────────────────────────────────────────────────────

      // ── Day-wise break from cached_shift_schedule (primary source) ────────
      // Shift schedule ki tarah har din alag break time.
      // Agar aaj ka din schedule mein mila → return (API call skip).
      // Agar nahi mila (7 days mein se bhi nahi) → neeche purani API fallback.
      final bool foundInSchedule = await _applyTodayBreakFromSchedule();
      if (foundInSchedule) return;
      // ─────────────────────────────────────────────────────────────────────

      debugPrint('[BreakSchedule] ⬇️ Using attendancedata API as fallback (schedule miss)');
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // START BREAK  ← ✅ MODIFIED: clock-in check added at the very beginning
  // ══════════════════════════════════════════════════════════════════════════
  Future<void> startBreak({bool autoTriggered = false}) async {
    ...
    if (!isClockedIn) {
      ...
      debugPrint('❌ [BreakVM] startBreak BLOCKED — user is not clocked in');
      return;
    }
    // ──────────────────────────────────────────────────────────────────────────
