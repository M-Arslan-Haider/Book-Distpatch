...
//         );
//         _loginRepository.preloadOfflineData(employeeId, currentCompanyCode.value);
//
//         if (wasRestored) {
...
import '../Repositories/LoginRepositories/login_repository.dart';
import '../Services/biometric_service.dart';
import '../Services/data_preload_service.dart'; // ✅ NAYA IMPORT
import '../constants.dart';
import '../ViewModels/attendance_view_model.dart';
...
class LoginViewModel extends GetxController {
  ...
  Future<bool> login(String employeeId, String password) async {
    try {
      ...
      if (result.isSuccess) {
        ...
        // ✅ OFFLINE DATA PRELOAD - Sab data cache ho jayega
  ...
  Future<void> logout() async {
    ...
    await prefs.remove('emp_id');
    await prefs.remove('geoFencing');
    await prefs.remove(prefIsTimekeeper);
    await prefs.setBool(prefIsAuthenticated, false);

    currentUser.value  = null;
    isTimekeeper.value = false;
    _cachedEmpId       = '';
    _cachedPassword    = '';
