...
//         posted        : 0,
//         body          : Uint8List.fromList(bytes),
//         company_code  : DBHelper.getCompanyCode(), // ← ADDED
//       );
//
//       await _repo.add(model);
//       await fetchAll();
...
// ──────────────────────────────────
// 1. GPS accuracy filter  – points with accuracy > 50 m are dropped entirely.
// 2. Outlier speed filter – points implying > 180 km/h are dropped.
// 3. Kalman-style smoothing via _SmoothedPosition (α = 0.3) – eliminates
//    GPS zigzag that inflated distance by 30–200 %.
// 4. <name> tag in _blankGpx (was <n>) – valid GPX so re-parse after
//    app restart no longer silently returns 0 km.
// 5. Heartbeat points are marked with name="heartbeat" AND are excluded
//    from distance accumulation (they carry no movement).
// 6. _saveLocationRecord no longer double-counts: it sums only *previous*
//    sessions (posted == 1 or locationId != current), then adds the fresh
//    distance for this session only.
// 7. onClockIn resets totalDistance.value = 0 AND clears the Kalman state.
// 8. _initGpxFile on restore: starts a NEW segment so stale heartbeat
//    points from the old session never add phantom distance.
// 9. distanceFilter raised to 5 m (was 10 m) – better for slow walking.
//10. getImmediateDistance now prefers in-memory totalDistance.value while
//    clocked-in (no file I/O on every UI tick).
...
// ─────────────────────────────────────────────────────────────────────────────
// Simple exponential-smoothing filter for lat/lon
// α = 0.3  →  new_est = α·raw + (1-α)·prev_est
// ─────────────────────────────────────────────────────────────────────────────
class _SmoothedPosition {
  ...
  double? _lat;
  double? _lon;
  bool get isInitialized => _lat != null;

  /// Returns smoothed (lat, lon) pair.
  (double, double) update(double rawLat, double rawLon) {
    if (_lat == null) {
      _lat = rawLat;
      _lon = rawLon;
    } else {
      _lat = _alpha * rawLat + (1 - _alpha) * _lat!;
      _lon = _alpha * rawLon + (1 - _alpha) * _lon!;
    }
    return (_lat!, _lon!);
  }
...
class LocationViewModel extends GetxController {
  ...
  var totalDistance = 0.0.obs;   // km – live in-memory accumulator (FIX #10)
  var isLoading     = false.obs;
  var lastSyncTime  = ''.obs;
  ...
  // ── Smoothing filter (FIX #2, #3) ────────────────────────────────────────
  final _smoother = _SmoothedPosition();
  ...
  // ── GPS quality thresholds (FIX #1, #2) ──────────────────────────────────
  static const double _maxAccuracyMeters = 50.0;  // drop if worse than 50 m
  static const double _maxSpeedKmh       = 180.0; // drop if implies > 180 km/h
  ...
  Future<void> onClockIn() async {
    ...
    // FIX #7: reset everything cleanly
    totalDistance.value = 0.0;
    secondsPassed.value = 0;
    _lastTrackPoint     = null;
    _gpxInitialised     = false;
    _cachedDistance     = null;
    _smoother.reset();          // FIX #3
    isClockedIn.value   = true;
  ...
  Future<void> onClockOut() async {
    ...
    // FIX #3/#5: use the in-memory accumulator (already filtered/smoothed)
    // Re-calculate from file as authoritative source at clock-out
    final distance = _gpxInitialised && _gpxFile != null
        ? await _calculateDistanceFromFile(_gpxFile!.path)
        : totalDistance.value;
    totalDistance.value = distance;
  ...
  /// FIX #10: while clocked-in, return in-memory value immediately.
  /// Expensive file I/O only happens when not actively tracking.
  Future<double> getImmediateDistance() async {
  ...
  Future<double> calculateShiftDistance(DateTime shiftStart) async {
    try {
      ...
      for (final trk in gpx.trks) {
        for (final seg in trk.trksegs) {
          // FIX #5: exclude heartbeat points from distance calc
          final pts = seg.trkpts
              .where((p) =>
          p.time != null &&
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // PUBLIC – SAVE LOCATION FROM CONSOLIDATED FILE
  // ─────────────────────────────────────────────────────────────────────────
      ...
      if (await _gpxFile!.exists()) {
        ...
        if (content.isNotEmpty) {
          try {
            ...
            if (restore) {
              // FIX #8: on app-restart restore, start a FRESH segment so
              // stale heartbeat / duplicate points are never measured again.
              _segment = Trkseg();
              _track!.trksegs.add(_segment!);
            } else {
              _segment = _track!.trksegs.isNotEmpty
...
<gpx version="1.1" creator="GPS_Attendance">
  ...
  void _startPositionStream() {
    ...
    final settings = AndroidSettings(
      accuracy        : LocationAccuracy.high,
      distanceFilter  : 5,                         // FIX #9: 5 m (was 10)
  ...
  void _handlePosition(Position pos) {
    ...
    // ── FIX #1: accuracy filter ──────────────────────────────────────────
    if (pos.accuracy > _maxAccuracyMeters) {
      debugPrint('⚠️ [LocVM] Dropped low-accuracy point '
          '(acc=${pos.accuracy.toStringAsFixed(1)} m)');
      return;
    }

    // ── FIX #3: smooth lat/lon ───────────────────────────────────────────
    final (smoothLat, smoothLon) = _smoother.update(pos.latitude, pos.longitude);

    // ── FIX #2: speed-based outlier filter ──────────────────────────────
    if (_lastTrackPoint != null) {
      final distM = Geolocator.distanceBetween(
        _lastTrackPoint!.latitude, _lastTrackPoint!.longitude,
        smoothLat, smoothLon,
      );
      final dtSec = pos.timestamp != null && _lastTrackPoint!.timestamp != null
          ? pos.timestamp!.difference(_lastTrackPoint!.timestamp!).inSeconds
          .abs()
          : 1;
      if (dtSec > 0) {
        final speedKmh = (distM / dtSec) * 3.6;
        if (speedKmh > _maxSpeedKmh) {
          debugPrint('⚠️ [LocVM] Dropped outlier point '
    ...
    // FIX #5: only movement points accumulate distance
    if (_segment!.trkpts.length >= 2) {
      final prev = _segment!.trkpts[_segment!.trkpts.length - 2];
      totalDistance.value += _distBetween(prev, wpt);
      _cachedDistance      = null;
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // PRIVATE – FORCED POINT TIMER  (heartbeat – time continuity only)
  // ─────────────────────────────────────────────────────────────────────────
  ...
  void _startForcedPointTimer() {
    ...
    _forcedPointTimer = Timer.periodic(
      const Duration(seconds: 60),
        ...
        // FIX #5: heartbeat carries name='heartbeat' so distance calc skips it
        _segment!.trkpts.add(Wpt(
          lat : _lastTrackPoint!.latitude,
          lon : _lastTrackPoint!.longitude,
          time: DateTime.now(),
  ...
  Future<void> _saveLocationRecord({required double distance}) async {
    try {
      ...
      // FIX #6: sum only *already posted* records for today (previous sessions)
      // so we never double-count the current session's live distance.
      final todayRecords = await _repo.getByDate(dateStr);
      final previousTotal = todayRecords
          .where((r) => r.posted == 1) // only completed sessions
          .fold<double>(
          0.0, (sum, r) => sum + (double.tryParse(r.totalDistance) ?? 0.0));
      final cumulativeDistance = previousTotal + distance;

      debugPrint(
          '📊 [LocVM] Distance accumulation → '
              'previous sessions: ${previousTotal.toStringAsFixed(3)} km + '
              'this session: ${distance.toStringAsFixed(3)} km = '
              'cumulative: ${cumulativeDistance.toStringAsFixed(3)} km');
  ...
  // In location_view_model.dart, update the _buildLocationId method:
  ...
  String _buildLocationId({required String empId}) {
    ...
    if (companyCode.isNotEmpty) {
      id = '$companyCode-LOC-EMP-$empPart-$day-$month-$serial';
    } else {
      id = 'LOC-EMP-$empPart-$day-$month-$serial';
    }
  ...
  /// FIX #5: skips heartbeat points so they never contribute to distance.
  Future<double> _calculateDistanceFromFile(String path) async {
    try {
      final file = File(path);
      if (!await file.exists()) return 0.0;
      ...
      for (final trk in gpx.trks) {
        for (final seg in trk.trksegs) {
          // exclude heartbeat points from file-based distance calculation
          final pts = seg.trkpts
              .where((p) => p.name != 'heartbeat')
              .toList();
          for (int i = 0; i < pts.length - 1; i++) {
            total += _distBetween(pts[i], pts[i + 1]);
          }
  ...
  Future<void> _restoreClockedInState() async {
    ...
    final empId = _readPref(prefs, 'emp_id');
    isClockedIn.value = true;
    // FIX #8: pass restore:true so a new segment is started
