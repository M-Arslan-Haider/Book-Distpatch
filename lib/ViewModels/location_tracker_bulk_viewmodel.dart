...
//
//   static const String   _endpoint          = 'http://103.120.70.171:8001/location/bulk';
...
//
//       debugPrint('⏱️ [BULK] tick at ${DateTime.now().toIso8601String()} | isClockedIn=$isClockedIn');
//
//       if (!isClockedIn) {
//         if (_wasClockedIn) {
...
//       }
//
//       // Validate lat/lng — treat near-zero/sentinel values as invalid (matches Kotlin)
//       if ((position.latitude == 0.0 && position.longitude == 0.0) ||
//           (position.latitude > -0.001 && position.latitude < 0.001 &&
//               position.longitude > -0.001 && position.longitude < 0.001)) {
//         debugPrint('⚠️ [BULK] SKIP — invalid/near-zero coordinates: '
//             'lat=${position.latitude} lng=${position.longitude}');
//         return;
//       }
//
//       // Accuracy filter
//       if (position.accuracy == 0.0 || position.accuracy > _maxAccuracyMeters) {
...
//   ///replace
//   bool _readClockInState(SharedPreferences prefs) {
//     return prefs.getBool('flutter.isClockedIn') ??
//         prefs.getBool('flutter.clockedIn') ??
...
//     }
//
//     // Increased timeout to 12s to reduce missed ticks on slow devices
//     return Geolocator.getCurrentPosition(
//       desiredAccuracy: LocationAccuracy.high,
...
//           'prefs_keys=${prefs.getKeys().toList()}');
//       return null;
//     }
//
...
///
/// ALL FIXES applied in this version:
///   1. Timer interval: DYNAMIC — read from SharedPreferences (server-driven,
///      key: gps_post_interval_seconds), falls back to 60s default. Not a
///      fixed Timer.periodic anymore — a self-rescheduling loop, so every
///      single cycle re-reads the latest value from prefs. Works the same
///      whether the app is foreground, background, or was killed and its
///      process restarted from Recents — because the value always comes
///      fresh from disk (SharedPreferences / native SharedPrefs on Android),
///      never from an in-memory constant.
///   2. Accuracy filter: skip GPS readings worse than 50m (unchanged)
///   3. Distance filter REMOVED — stationary users ka data bhi capture ho
///   4. SQLite-first architecture — RAM buffer hataya, har record pehle SQLite
///      mein save hota hai, phir server ko sync hota hai (100% data safety)
...
class LocationBulkTracker {
  ...
  // ✅ Same pref key LoginRepository writes to on login / clock-in refresh.
  // Native (Kotlin) side should read this same key so foreground-service
  // ticking (LocationMonitorService) and this Dart tracker always agree.
  static const String _prefIntervalKey        = 'gps_post_interval_seconds';
  static const int    _defaultIntervalSeconds = 60;
  static const int    _minIntervalSeconds     = 5; // matches server CHK_GPS_CFG_INTERVAL
  ...
  bool   _wasClockedIn   = false;
  bool   _syncInProgress = false;
  int    _loopGeneration = 0; // ✅ bump on stop() so any in-flight scheduled loop dies quietly
  ...
  /// Reads the current interval fresh from disk every time it's called.
  /// This is what makes the interval "dynamic" — no cached Duration field.
  Future<int> _currentIntervalSeconds() async {
    final prefs = await SharedPreferences.getInstance();
    // reload() ensures we see a value written by another isolate/process
    // (e.g. the native LocationMonitorService or a background handler)
    // even within the same app session.
    await prefs.reload();
    final raw = prefs.getInt(_prefIntervalKey);
    if (raw == null || raw < _minIntervalSeconds) {
  ...
  Future<void> start() async {
    ...
    final int startInterval = await _currentIntervalSeconds();
    debugPrint('✅ [BULK] Tracker started | '
        'interval=${startInterval}s (dynamic, re-read every cycle) | '
        'maxAccuracy=${_maxAccuracyMeters}m | '
        'distanceFilter=DISABLED | '
        'serialCounter=$_locationSerialCounter');
    ...
    // ✅ Self-rescheduling loop instead of Timer.periodic(fixedDuration, ...).
    // After every tick, re-reads the interval from SharedPreferences and
    // schedules the NEXT tick with whatever value is current at that moment.
    // This means:
    //   - Value changed at login → next loop picks it up immediately.
    //   - Value changed via Clock-In refresh → next loop picks it up immediately.
    //   - App backgrounded/foregrounded/relaunched from Recents → loop restarts
    //     via start() (see MainActivity/LocationMonitorService lifecycle hooks)
    //     and reads the same persisted value — no drift, no stale hardcoded 10s.
    _runLoop(myGeneration);
  }

  void _runLoop(int generation) {
    Future<void> cycle() async {
      if (generation != _loopGeneration || !_isRunning) return; // superseded/stopped

      await _tick();

      if (generation != _loopGeneration || !_isRunning) return; // stopped during tick

      final int nextIntervalSeconds = await _currentIntervalSeconds();
      Future.delayed(Duration(seconds: nextIntervalSeconds), () {
        unawaited(cycle());
      });
    }
  ...
  Future<void> stopAndFlush() async {
    debugPrint('🛑 [BULK] Stop requested');
    _loopGeneration++; // invalidates any pending Future.delayed callback
    _isRunning = false;
  ...
  Future<void> _tick() async {
    ...
    try {
      final SharedPreferences prefs = await SharedPreferences.getInstance();
      final bool isClockedIn        = _readClockInState(prefs);
      final int  activeInterval     = await _currentIntervalSeconds();

      debugPrint('⏱️ [BULK] tick at ${DateTime.now().toIso8601String()} | '
          'isClockedIn=$isClockedIn | interval=${activeInterval}s');
      ...
      // ✅ FIX: Sync every 1 record — real-time posting
      if (pendingCount >= 1) {
        await _syncFromDb(reason: 'real-time-post');
      }
    } catch (e) {
      debugPrint('❌ [BULK] Tick error: $e');
    } finally {
