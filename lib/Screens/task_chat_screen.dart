...
class _ChatMsg {
  ...
  factory _ChatMsg.fromJson(Map<String, dynamic> json, String currentEmpId, [String currentName = '']) {
    ...
    final isMobileUpload = loggedType?.toUpperCase() == 'MOBILE';
    final isMe = !isAdmin &&
        (loggedBy == currentEmpId ||
            (currentName.isNotEmpty &&
                loggedBy.trim().toLowerCase() ==
                    currentName.trim().toLowerCase()) ||
            (isMobileUpload && loggedBy.isEmpty));
    ...
    // FIX: Better name resolution to avoid "Unknown"
    String name;
    if (isAdmin) {
      name = 'Admin';
    } else if (isMe) {
      name = currentName.isNotEmpty ? currentName :
      (json['employee_name'] as String? ?? 'Me');
    } else {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final parsed = items
            ...
            Map<String, dynamic>.from(e as Map), _currentEmpId, _currentName))
...
// ═══════════════════════════════════════════════════════════════
//  FULL-SCREEN IMAGE VIEWER (FIXED - Centered position)
// ═══════════════════════════════════════════════════════════════
    ...
    return Scaffold(
      ...
      body: Stack(
        ...
        children: [
          ...
          // otherwise an unconstrained child renders at its natural size and
          // anchors to the top-left instead of the center.
          Positioned.fill(
            child: SafeArea(
              child: LayoutBuilder(
