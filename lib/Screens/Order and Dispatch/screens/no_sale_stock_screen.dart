...
//
//   /// Required fields: Brand, Shop Photo, Location.
//   /// Stock items list is intentionally NOT required — it can stay empty/null.
//   bool get _canSubmit {
...
//
//     if (ok) {
//       // Clear saved stock quantities for this shop so the next visit/order
//       // starts fresh instead of prefilling the quantities from this visit.
...
//           content: Text(
//             'Stock visit for ${_controller.model.shopName} was saved successfully.',
...
//                   ..pop() // NoSaleStockScreen
//                   ..pop(); // NoSaleShopSelectScreen
//               },
...
class _NoSaleStockScreenState extends State<NoSaleStockScreen> {
  ...
  Future<void> _openQuantitySheet(StockCatalogProduct product) async {
    ...
    if (initialQty <= 0) {
      ...
      initialQty = await loadStockQtyFromPrefs(
        shopId: shopId,
        productName: product.name.trim(),   // ✅ trim add karo
      );
    }
  ...
  Future<void> _goToOrder() async {
    ...
    // ✅ Submit visit karo — stock clear nahi hoga (controller me clear hai,
    // lekin hum _goToOrder ke andar extra clear nahi karenge)
    final ok = await _controller.submitVisit();
    if (!mounted) return;
