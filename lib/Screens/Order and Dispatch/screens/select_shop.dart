...
//       }
//
//       // ✅ FIXED: Correct URL format with path parameters
//       // URL: /addshopget/get/:emp_id/:company_code
//       String endpoint;
//       if (_companyCode.isNotEmpty) {
//         endpoint = '/addshopget/get/$_empId/$_companyCode';
//       } else {
//         endpoint = '/addshopget/get/$_empId';
//       }
//
//       debugPrint('📤 [SelectShop] GET $_baseUrl$endpoint');
//       final response = await http.get(Uri.parse('$_baseUrl$endpoint'));
//
//       if (response.statusCode == 200) {
//         final data = jsonDecode(response.body);
//
//         // Check if data is a list (from your DECLARE.txt output)
//         final List<dynamic> items;
//         if (data is List) {
//           items = data;
//         } else {
//           // If it's wrapped in an object with 'items' key
...
//           _loading = false;
//         });
//         debugPrint('❌ [SelectShop] API error: ${response.statusCode} - ${response.body}');
//       }
//     } catch (e) {
...
class _SelectShopSheetState extends State<SelectShopSheet> {
  ...
  Future<void> _refreshShopsInBackground() async {
    try {
      ...
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        final List<dynamic> items = data is List ? data : (data['items'] as List? ?? []);
        final prefs = await SharedPreferences.getInstance();
        await prefs.setString('${DataPreloadService.KEY_SHOPS_PREFIX}$_empId', jsonEncode(items));
        debugPrint('🔄 [SelectShop] Shops refreshed in background');
      }
    } catch (e) {
      debugPrint('⚠️ [SelectShop] Background shop refresh failed: $e');
    }
  }
