...
///offline
// ═══════════════════════════════════════════════════════════════════════════
// customer_account_screen.dart - Updated with offline support
// ═══════════════════════════════════════════════════════════════════════════
...
class _CustomerAccountScreenState extends State<CustomerAccountScreen> {
  ...
  void initState() {
    ...
    // Try to load cached account data first (for immediate display)
    _loadCachedAccountData();
  }
  ...
  Future<void> _loadCachedAccountData() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final cachedKey = 'cached_account_${widget.shopId}';
      final cachedData = prefs.getString(cachedKey);

      if (cachedData != null && cachedData.isNotEmpty) {
        final Map<String, dynamic> data = jsonDecode(cachedData);
        final account = CustomerAccount(
          shopName: data['shopName'] ?? widget.shopName,
          subtitle: data['subtitle'] ?? widget.shopSubtitle,
          ledgerBalance: (data['ledgerBalance'] ?? 0).toDouble(),
          remainingBillLimit: (data['remainingBillLimit'] ?? 0).toDouble(),
          lastPaymentDate: data['lastPaymentDate'] != null ? DateTime.parse(data['lastPaymentDate']) : null,
          paymentDue: (data['paymentDue'] ?? 0).toDouble(),
        );
        // Set the account data immediately (will be replaced by fresh data when loaded)
        _controller.model.account = account;
        _controller.refreshUI();
        print('📦 [CustomerAccountScreen] Loaded cached account data');
      }
    } catch (e) {
      print('⚠️ [CustomerAccountScreen] Error loading cached account: $e');
    }
  }
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                if (model.isLoadingAccount && model.account == null)
                  const Padding(
                    padding: EdgeInsets.symmetric(vertical: 40),
                    child: Center(child: CircularProgressIndicator(color: _tealDark)),
                  )
