...
class _AddShopScreenState extends State<AddShopScreen> {
  ...
  Future<bool> _hasInternet() async {
    try {
      final result = await InternetAddress.lookup('oracle.metaxperts.net');
  ...
  Future<void> _fetchCities() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final cachedData = prefs.getString(DataPreloadService.KEY_CITIES);
      ...
      if (cachedData != null && cachedData.isNotEmpty) {
        ...
        debugPrint('📦 [AddShop] Loaded ${_cities.length} cities from cache');
      ...
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        final items = data['items'] as List? ?? [];
        await prefs.setString(DataPreloadService.KEY_CITIES, jsonEncode(items));
        setState(() {
          _cities = items.map((e) => e['city'].toString()).toList();
          _loadingCities = false;
        });
        debugPrint('✅ [AddShop] Fetched ${_cities.length} cities from API');
      } else {
        setState(() => _loadingCities = false);
      }
    } catch (e) {
      debugPrint('⚠️ [AddShop] Cities error: $e');
      try {
        final prefs = await SharedPreferences.getInstance();
        final cachedData = prefs.getString(DataPreloadService.KEY_CITIES);
        if (cachedData != null && cachedData.isNotEmpty) {
          ...
          debugPrint('📦 [AddShop] Fallback to cached cities (${_cities.length})');
        } else {
          setState(() => _loadingCities = false);
        }
      } catch (_) {
  ...
  Future<void> _refreshCitiesInBackground() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final prefs = await SharedPreferences.getInstance();
        await prefs.setString(DataPreloadService.KEY_CITIES, jsonEncode(items));
        debugPrint('🔄 [AddShop] Cities refreshed in background');
      }
    } catch (e) {
      // ignore
    }
  ...
  void _showSuccess({required bool isOffline}) {
    Get.snackbar(
      '✅ Saved',
      isOffline
          ? 'Shop saved locally (will sync when online)'
          : 'Shop saved as pending approval — bookable on hold.',
      snackPosition:   SnackPosition.BOTTOM,
      backgroundColor: _primary,
      colorText:       Colors.white,
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // ✅ UPDATED: OFFLINE QUEUE - SHOP SAVE HAMESHA SUCCESS
  // ═══════════════════════════════════════════════════════════════════════
  Future<void> _submit() async {
    if (_empId.isEmpty || _empName.isEmpty) {
      await _loadEmployeeInfo();
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        // ✅ Online success
        _clearForm();
        _showSuccess(isOffline: false);
        Future.delayed(const Duration(seconds: 1), () => Get.back());
        return;
      }
    } catch (e) {
      debugPrint('⚠️ [AddShop] Online submit failed, queueing offline: $e');
      isOffline = true;
    }

    // ✅ OFFLINE: Queue the shop for later sync
    await _queuePendingShop(payload);

    // ✅ Always show success - smooth user experience
    _clearForm();
    _showSuccess(isOffline: true);
    Future.delayed(const Duration(seconds: 1), () => Get.back());
  }

  // ── Queue shop in SharedPreferences ──────────────────────────────────────
  Future<void> _queuePendingShop(Map<String, dynamic> payload) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final queueKey = 'pending_shops_${_empId}';
      ...
      queue.removeWhere((item) {
        try {
          ...
          return data['shop_id'] == payload['shop_id'];
        } catch (_) {
          return false;
        }
      });
      ...
      debugPrint('📦 [AddShop] Shop queued: ${payload['shop_id']}');
      debugPrint('📦 [AddShop] Total pending shops in queue: ${queue.length}');
    } catch (e) {
      debugPrint('💥 [AddShop] Error queueing shop: $e');
    }
  }
