...
//   }
//
//   // ── Selfie Capture (Enhanced with better compression) ────────────────────
//   Future<String?> _captureSelfie() async {
//     debugPrint('📸 [SELFIE] Starting selfie capture...');
//     try {
//       final ImagePicker picker = ImagePicker();
...
//         source: ImageSource.camera,
//         preferredCameraDevice: CameraDevice.front,
//         imageQuality: 50, // Reduced from 80 to 50 for smaller size
//       );
//
//       if (photo == null) {
//         debugPrint('📸 [SELFIE] User cancelled — no photo taken');
...
//       bytes = await _compressImageIfNeeded(bytes);
//
//       // Additional emergency compression if still too large
//       const int absoluteMaxBytes = 30 * 1024; // 30 KB absolute max
//       if (bytes.length > absoluteMaxBytes) {
//         debugPrint('📸 [SELFIE] Still too large (${(bytes.length / 1024).toStringAsFixed(1)} KB), applying emergency compression...');
//         bytes = await _emergencyCompress(bytes);
//       }
//
//       if (bytes.isEmpty) {
//         debugPrint('❌ [SELFIE] Bytes are 0');
...
//
//       // Final size check
//       debugPrint('📸 [SELFIE] Final selfie size: ${(bytes.length / 1024).toStringAsFixed(2)} KB');
//
//       // Convert to base64 for API
//       final base64Image = base64Encode(bytes);
//       debugPrint('📸 [SELFIE] Base64 length: ${base64Image.length} chars (${(base64Image.length / 1024).toStringAsFixed(1)} KB)');
//
//       if (base64Image.length > 40000) {
//         debugPrint('⚠️ [SELFIE] WARNING: Base64 is ${(base64Image.length / 1024).toStringAsFixed(1)} KB - may exceed database column limit');
//       }
//
//       return base64Image;
//     } catch (e) {
...
//   }
//
//   // ── Enhanced Image Compression Helper ──────────────────────────────────────
//   Future<Uint8List> _compressImageIfNeeded(Uint8List bytes) async {
//     const int maxBytes = 25 * 1024; // Reduced to 25 KB for database compatibility
//
//     if (bytes.length <= maxBytes) {
//       debugPrint('📸 [COMPRESS] Image within limit — no compression needed');
//       return bytes;
//     }
//
//     debugPrint('📸 [COMPRESS] Image exceeds 25 KB (${(bytes.length / 1024).toStringAsFixed(1)} KB) — compressing...');
//
//     try {
//       Uint8List current = bytes;
//       int attempt = 0;
//       const int maxAttempts = 8; // Increased attempts
//
//       while (current.length > maxBytes && attempt < maxAttempts) {
//         attempt++;
//
//         final codec = await ui.instantiateImageCodec(current);
//         final frame = await codec.getNextFrame();
//         final image = frame.image;
//
//         // More aggressive scaling with each attempt
//         double scale = (maxBytes / current.length);
//         if (attempt == 1) scale = scale * 0.9;
//         else if (attempt == 2) scale = scale * 0.7;
//         else if (attempt >= 3) scale = scale * 0.5;
//         scale = scale.clamp(0.05, 0.9);
//
//         final int newWidth = (image.width * scale).toInt().clamp(30, image.width);
//         final int newHeight = (image.height * scale).toInt().clamp(30, image.height);
//
//         final recorder = ui.PictureRecorder();
//         final canvas = Canvas(recorder);
//         canvas.drawImageRect(
...
//   Future<Uint8List> _emergencyCompress(Uint8List bytes) async {
//     try {
//       debugPrint('🚨 [EMERGENCY] Applying emergency compression...');
//       final codec = await ui.instantiateImageCodec(bytes);
//       final frame = await codec.getNextFrame();
//       final image = frame.image;
//
//       // Aggressive resize to very small dimensions
//       final int newWidth = (image.width * 0.2).toInt().clamp(40, image.width);
//       final int newHeight = (image.height * 0.2).toInt().clamp(40, image.height);
//
//       final recorder = ui.PictureRecorder();
//       final canvas = Canvas(recorder);
//       canvas.drawImageRect(
...
//         debugPrint('🚨 [EMERGENCY] Compressed to ${(compressed.length / 1024).toStringAsFixed(1)} KB');
//         return compressed;
//       }
//     } catch (e) {
//       debugPrint('❌ [EMERGENCY] Emergency compression failed: $e');
//     }
//     return bytes;
//   }
//
...
//     }
//
//     // Validate selfie size before sending
//     final selfieSizeKB = selfieBase64.length / 1024;
//     if (selfieSizeKB > 50) {
//       debugPrint('⚠️ [CLOCK-IN] Selfie base64 is ${selfieSizeKB.toStringAsFixed(1)} KB - may be too large');
//       _showSnack(
//         '⚠️ Selfie too large (${selfieSizeKB.toStringAsFixed(0)} KB). Please try again.',
//         const Color(0xFFD97706),
//       );
//       return;
//     }
//
//     debugPrint('✅ [CLOCK-IN] Selfie captured successfully (${selfieBase64.length} chars, ${selfieSizeKB.toStringAsFixed(1)} KB)');
//
//     setState(() => _clockInLoading.add(wagerId));
//
//     try {
...
//
//       final bodyJson = jsonEncode(body);
//       debugPrint('📦 [CLOCK-IN] Total request body size: ${(bodyJson.length / 1024).toStringAsFixed(1)} KB');
//
//       debugPrint('╔══ CLOCK IN POST ═══════════════════════════════');
//       debugPrint('║ URL: http://oracle.metaxperts.net/ords/gps_workforce/wagerattendancein/post/');
//       debugPrint('║ Wager: $wagerName ($wagerId)');
//       debugPrint('║ ID: $attendanceId');
//       debugPrint('║ Selfie Length: ${selfieBase64.length} chars');
//       debugPrint('║ Selfie Size: ${(selfieBase64.length / 1024).toStringAsFixed(1)} KB');
//       debugPrint('║ LatLng: $lat, $lng');
//       debugPrint('╚════════════════════════════════════════════════');
//
//       debugPrint('🟢 [CLOCK-IN] Sending POST…');
...
//     final selfieSizeKB = selfieBase64.length / 1024;
//     if (selfieSizeKB > 50) {
//       debugPrint('⚠️ [CLOCK-OUT] Selfie base64 is ${selfieSizeKB.toStringAsFixed(1)} KB - may be too large');
//       _showSnack(
//         '⚠️ Selfie too large (${selfieSizeKB.toStringAsFixed(0)} KB). Please try again.',
//         const Color(0xFFD97706),
//       );
...
//     }
//
//     debugPrint('✅ [CLOCK-OUT] Selfie captured successfully (${selfieBase64.length} chars, ${selfieSizeKB.toStringAsFixed(1)} KB)');
//
//     setState(() => _clockOutLoading.add(wagerId));
//
//     try {
...
//
//       final bodyJson = jsonEncode(body);
//       debugPrint('📦 [CLOCK-OUT] Total request body size: ${(bodyJson.length / 1024).toStringAsFixed(1)} KB');
//
//       debugPrint('╔══ CLOCK OUT POST ══════════════════════════════');
//       debugPrint('║ URL: http://oracle.metaxperts.net/ords/gps_workforce/wagerattendanceout/post/');
//       debugPrint('║ Wager: $wagerName ($wagerId)');
...
class _WagersAttendanceScreenState extends State<WagersAttendanceScreen> {
  ...
  Future<String?> _captureSelfie() async {
    ...
    try {
      ...
      const int absoluteMaxBytes = 30 * 1024;
      if (bytes.length > absoluteMaxBytes) {
        debugPrint('📸 [SELFIE] Still too large (${(bytes.length / 1024).toStringAsFixed(1)} KB), applying emergency compression...');
        bytes = await _emergencyCompress(bytes);
      }
      ...
      body: Column(
        ...
        children: [
          ...
          _WagersGradientHeader(
            title:    'Wagers Attendance',
            subtitle: '${_filtered.length} active wager(s)',
