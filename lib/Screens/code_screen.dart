...
/// Auto-formats company code input as: XX-XXX-XXX-...
/// e.g. PK-PUN-SKT-MX04
class CompanyCodeFormatter extends TextInputFormatter {
  @override
  TextEditingValue formatEditUpdate(
    ...
    // Strip dashes + spaces, force uppercase
    final rawText = newValue.text
        .replaceAll('-', '')
        .replaceAll(' ', '')
        .toUpperCase();

    // Rebuild with auto dashes at positions 2, 5, 8 in raw text
    // Result pattern: XX-XXX-XXX-...
    ...
    // Auto-append trailing dash right after boundary (only while typing, not deleting)
    if (!isDeleting &&
        (rawText.length == 2 || rawText.length == 5 || rawText.length == 8)) {
      buffer.write('-');
    }
...
class _CodeScreenState extends State<CodeScreen>
  ...
  Future<void> _validateCompanyCode() async {
    ...
    try {
      ...
      // ✅ UPDATED: Using Remote Config for company validation URL
      final companyApiUrl = RemoteConfigService.getCompanyValidationUrl(companyCode);
      ...
      // ✅ UPDATED: Using Remote Config for login API URL
      await prefs.setString('login_api_url', RemoteConfigService.getLoginApiUrl(companyCode));
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    TextFormField(
                                      ...
                                      inputFormatters: [CompanyCodeFormatter()],
