...
import '../Services/developer_options_check_service.dart'; // ✅ NEW
...
class _HomeScreenState extends State<HomeScreen>
  ...
  void initState() {
    ...
    // ✅ Register lifecycle observer — resume par re-check ke liye
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _checkForUpdate();
      _checkDeveloperOptions(); // ✅ NEW — Developer Options check on home screen open
    });
  ...
  @override
  void dispose() {
    // ✅ Observer remove karo — memory leak se bachne ke liye
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // ✅ STRICT — App resume hone par HAMESHA developer options re-check karo
  //
  // Yeh method tab call hota hai jab:
  //   - User app minimize kar ke wapas ata hai
  //   - User settings se wapas ata hai
  //   - Kisi bhi reason se app foreground mein aata hai
  //
  // Har baar API POST bhi hoga (ON ho ya OFF dono cases mein).
  // ══════════════════════════════════════════════════════════════════════════
  @override
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // ✅ NEW — Developer Options Check
  // Home screen open hone par check karo.
  // ON hai → dialog show karo + API post karo.
  // Dialog mein koi cancel button nahi — sirf "Open Settings".
  // Clock-in flow mein koi change nahi.
  // ══════════════════════════════════════════════════════════════════════════
  Future<void> _checkDeveloperOptions() async {
    final result = await DeveloperOptionsCheckService.checkAndPost();
    if (result.isDeveloperOptionsEnabled && mounted) {
  ...
  void _showDeveloperOptionsDialog() {
    showDialog(
      context: context,
      barrierDismissible: false, // bahar tap karne se dismiss nahi hoga
      builder: (ctx) => PopScope(
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                // ── Open Settings Button (NO cancel button) ───────────────
                SizedBox(
                  width: double.infinity,
                  height: 50,
                  child: DecoratedBox(
                    ...
                    child: TextButton.icon(
                      onPressed: () {
                        ...
                        // Android Developer Options screen kholna
                        const MethodChannel(
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // UNIFIED SESSION CARD (unchanged)
  // ══════════════════════════════════════════════════════════════════════════
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  onBreak
                      ? (timerStr.isNotEmpty ? timerStr : 'Active')
                      : 'Short Break',
                  textAlign: TextAlign.center,
                  overflow: TextOverflow.ellipsis,
                  style: TextStyle(
                    fontSize: context.rs(onBreak && timerStr.isNotEmpty ? 12 : 13),
                    fontWeight: FontWeight.w600,
                    color: onBreak ? color : AppColors.textPrimary,
                  ),
                ),
                if (onBreak) ...[
                  SizedBox(height: context.rs(2)),
